...
export default function Header() {
  ...
  async function loadNotifications() {
    ...
    const [productsResult, receivablesResult] = await Promise.all([
      supabase.from("products").select("id,nama,stok").eq("brand_id", brand?.id ?? "").lte("stok", lowStock).order("stok", { ascending: true }).limit(10),
      supabase.from("transactions").select("id,invoice_number,total,paid_amount,customer_name").eq("brand_id", brand?.id ?? "").eq("payment_status", "BELUM_LUNAS").order("created_at", { ascending: false }).limit(10),
    ]);
