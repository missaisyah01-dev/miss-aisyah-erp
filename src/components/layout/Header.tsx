...
function EyeIcon({ hidden }: { hidden: boolean }) { return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4" aria-hidden="true"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z" /><circle cx="12" cy="12" r="2.5" />{hidden && <path d="m4 4 16 16" />}</svg>; }
...
export default function Header() {
  ...
  const email = user?.email || profile?.full_name || "Pengguna";
  ...
  async function loadNotifications() {
    ...
    if (!receivablesResult.error) (receivablesResult.data ?? []).forEach((transaction) => {
      ...
      next.push({ id: `receivable-${transaction.id}`, title: "Piutang belum lunas", description: `${transaction.invoice_number}${transaction.customer_name ? ` · ${transaction.customer_name}` : ""} · sisa Rp ${remaining.toLocaleString("id-ID")}.`, tone: "violet", href: "/sales" });
    });
