...
export default function Sidebar() {
  ...
  return <>
    <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col overflow-y-auto bg-pink-600 p-6 text-white md:flex">
      <div className="flex items-center gap-3">{brand?.logo_url && <img src={brand.logo_url} alt={`Logo ${brand.name}`} onError={(event) => { event.currentTarget.style.display = "none"; }} className="h-12 w-12 shrink-0 rounded-xl bg-white/15 object-contain p-1" />}<div className="min-w-0"><h1 className="truncate text-2xl font-bold">{brand?.name ?? "ERP"}</h1><p className="mt-1 text-sm text-pink-100">Sistem Operasional Fashion</p></div></div>
