...
export default function SalesReport() {
  ...
  async function loadReport() {
    ...
    if (productsResult.error) alert(`Gagal memuat produk terlaris: ${productsResult.error.message}`);
    else setTopProducts((productsResult.data ?? []) as TopProduct[]);
    setLoading(false);
  }
  ...
  return <section className="space-y-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-3xl font-bold text-gray-900">Laporan Penjualan</h1><p className="mt-1 text-gray-500">Ringkasan performa penjualan MISS AISYAH.</p></div><select value={period} onChange={(event) => setPeriod(event.target.value as Period)} className="rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-gray-900 shadow-sm"><option value="HARI_INI">Hari ini</option><option value="MINGGU_INI">7 hari terakhir</option><option value="BULAN_INI">Bulan ini</option><option value="SEMUA">Semua waktu</option></select></div>
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]"><section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-bold text-gray-900">Tren Omzet Harian</h2><p className="mt-1 text-sm text-gray-500">Berdasarkan transaksi pada periode terpilih.</p>{loading ? <p className="py-20 text-center text-gray-500">Memuat laporan…</p> : chartItems.length === 0 ? <p className="py-20 text-center text-gray-500">Belum ada data penjualan pada periode ini.</p> : <div className="mt-7 flex h-60 items-end gap-2 border-b border-gray-200 pb-7">{chartItems.map((item) => <div key={item.date} className="flex h-full min-w-0 flex-1 flex-col justify-end"><p className="mb-2 truncate text-center text-xs font-medium text-gray-600">{formatRupiah(item.total)}</p><div title={`${shortDate(item.date)}: ${formatRupiah(item.total)}`} style={{ height: `${Math.max((item.total / largestDailyTotal) * 100, 5)}%` }} className="rounded-t-md bg-pink-500 transition hover:bg-pink-600" /><p className="mt-2 text-center text-xs text-gray-500">{shortDate(item.date)}</p></div>)}</div>}</section>
      <div className="space-y-6"><section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-bold text-gray-900">Metode Pembayaran</h2><div className="mt-5 space-y-5">{payments.map((payment) => <div key={payment.method}><div className="flex justify-between text-sm"><span className="font-medium text-gray-700">{payment.method === "CASH" ? "Tunai" : payment.method}</span><span className="font-semibold text-gray-900">{formatRupiah(payment.total)}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-gray-100"><div className="h-full rounded-full bg-pink-500" style={{ width: `${summary.omzet ? (payment.total / summary.omzet) * 100 : 0}%` }} /></div></div>)}</div></section><section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-bold text-gray-900">Produk Terlaris</h2>{loading ? <p className="py-8 text-center text-sm text-gray-500">Memuat…</p> : topProducts.length === 0 ? <p className="py-8 text-center text-sm text-gray-500">Belum ada produk terjual.</p> : <ol className="mt-4 space-y-3">{topProducts.map((product, index) => <li key={product.product_id} className="flex items-center gap-3"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-pink-100 text-xs font-bold text-pink-700">{index + 1}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-gray-900">{product.product_name}</p><p className="text-xs text-gray-500">{product.total_quantity} terjual · {formatRupiah(Number(product.total_revenue))}</p></div></li>)}</ol>}</section></div></div>
