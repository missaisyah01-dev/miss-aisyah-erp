...
export default function StockModal({ onClose }: { onClose: () => void }) {
  ...
  const [products, setProducts] = useState<Product[]>([]); const [productId, setProductId] = useState(""); const [type, setType] = useState("MASUK"); const [quantity, setQuantity] = useState(""); const [notes, setNotes] = useState(""); const [movementDate, setMovementDate] = useState(""); const [saving, setSaving] = useState(false);
